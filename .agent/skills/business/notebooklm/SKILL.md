...
### Step 5: Cross-Notebook Analysis (Multiple Notebooks)

When user wants to query multiple notebooks with the same question (e.g., for project hierarchies with sub-projects + overall project):

```bash
# Query ALL notebooks in library with the same question
python3 scripts/run.py ask_multiple.py --question "What are the main risks?" --all-notebooks

# Query SPECIFIC notebooks by ID
python3 scripts/run.py ask_multiple.py --question "GPU status?" --notebook-ids "id1,id2,id3"

# Show browser for debugging
python3 scripts/run.py ask_multiple.py --question "..." --all-notebooks --show-browser
```

**When to use Cross-Notebook Analysis:**
- User has project hierarchy (sub-projects + overall project notebooks)
- User wants comparative analysis across different documentation sets
- User asks to "check all notebooks", "ask all my docs", etc.
- User wants comprehensive view across entire knowledge base

**Output format:**
- Displays answers from each notebook separately
- Includes success/failure status for each
- Provides unified view for synthesis
- Same follow-up mechanism applies
...
```bash
python3 scripts/run.py ask_question.py --question "..." [--notebook-id ID] [--notebook-url URL] [--show-browser]
```

### Cross-Notebook Analysis (`ask_multiple.py`)
```bash
# Query all notebooks
python3 scripts/run.py ask_multiple.py --question "..." --all-notebooks

# Query specific notebooks by ID (comma-separated)
python3 scripts/run.py ask_multiple.py --question "..." --notebook-ids "id1,id2,id3"
...
All data stored in `.agent/skills/business/notebooklm/data/` (relative to project root):
...
7. **Cross-notebook for hierarchies** - Use `ask_multiple.py` for project structures with sub-projects
8. **Specific vs. all notebooks** - Use `--notebook-ids` for targeted analysis, `--all-notebooks` for comprehensive view
...
- `scripts/` - All automation scripts:
  - `ask_question.py` - Query single notebook
  - `ask_multiple.py` - Cross-notebook analysis
